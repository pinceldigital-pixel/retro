...
const prev=document.getElementById('prev'),playBtn=document.getElementById('play'),pauseBtn=document.getElementById('pause');
...
function updateTexts(){const st=stations[index];sub.textContent=st.freq.toFixed(1);big.textContent=st.freq.toFixed(1);nameEl.textContent=st.name.toUpperCase();}
function loadStation(auto=false){const st=stations[index];updateTexts();audio.src=st.url;if(auto)audio.play().catch(()=>{});if('mediaSession'in navigator){navigator.mediaSession.metadata=new MediaMetadata({title:st.name,artist:'AI Pixel Radio',album:'En vivo'});}}
prev.addEventListener('click',()=>{const was=!audio.paused;index=(index-1+stations.length)%stations.length;loadStation(was);});
playBtn.addEventListener('click',async()=>{try{await audio.play();startViz();}catch(e){}});
pauseBtn.addEventListener('click',()=>{audio.pause();stopViz();});
audio.addEventListener('play',()=>{'mediaSession'in navigator&&(navigator.mediaSession.playbackState='playing')});
